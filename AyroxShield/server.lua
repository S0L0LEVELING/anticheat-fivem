...
LogBanToDiscord = function (playerId,reason)
    ...
    local discordInfo = {
        ...
        ["footer"] = {
            ["text"] = '� AYROXSHIELD '..versionac
        }
    }

    PerformHttpRequest(ConfigACS.LogBanWebhook, function(err, text, headers) end, 'POST', json.encode({ username = '� AYROXSHIELD', embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
...
AddEventHandler('ayzn:detectionf748esf4esf4se85de7des7fesf5ede8sf', function(type, item)
    ...
    --else
        ...
        elseif (_type == 'godmode') then
            ...
            TriggerEvent("ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es", "� AYROXSHIELD : GodeMod",source)
        elseif (_type == 'resourcestart') then
            LogBanToDiscord(source,"Tried to start the resource ",item)
            TriggerEvent("ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es", "� AYROXSHIELD : Resource Start",source)
        elseif (_type == 'resourcestop') then
            LogBanToDiscord(source,"Tried to stop the resource ",item)
            TriggerEvent("ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es", "� AYROXSHIELD : Resource Stop",source)
        ...
        elseif (_type == 'spec') then
            ...
            TriggerEvent("ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es", "� AYROXSHIELD : Anti Spectate",source)
        elseif (_type == 'resourcecounter') then
            LogBanToDiscord(source,"has a different resource number count")
            TriggerEvent("ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es", "� AYROXSHIELD : Different Resource Count",source)
        elseif (_type == 'antiblips') then
            LogBanToDiscord(source,"tried to enable players blips")
            TriggerEvent("ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es", "� AYROXSHIELD : Anti-Blips",source)
        elseif (_type == 'injection') then
            LogBanToDiscord(source,"tried to execute the command "..item)
            TriggerEvent("ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es", "� AYROXSHIELD : Blacklisted Command",source)
        ...
        elseif (_type == 'hash') then
            ...
            TriggerServerEvent("ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es", "� AYROXSHIELD : Blacklisted Car",source)
        elseif (_type == 'explosion') then
            LogBanToDiscord(source,"Tried to spawn an explosion : "..item)
            TriggerServerEvent("ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es", "� AYROXSHIELD : Spawn Explosion",source)
        elseif (_type == 'event') then
            LogBanToDiscord(source,"Tried to trigger a blacklisted event : "..item)
            TriggerEvent("ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es", "� AYROXSHIELD : Blacklisted Event",source)
        end
    --end
end)
    ...
    AddEventHandler("explosionEvent",function(sender, ev)
        for _, v in ipairs(ConfigACC.BlockedExplosions) do
            if ev.explosionType == v then
                ...
                TriggerEvent("ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es", "� AYROXSHIELD : Spawned Explosion",sender)
                return
            end
        end
    end)
...
if ConfigACC.GiveWeaponsProtection then
    AddEventHandler("giveWeaponEvent", function(sender, data)
        ...
        TriggerEvent("ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es", "� AYROXSHIELD : Give Weapon",sender)
    end)
end
...
if ConfigACC.WordsProtection then
    AddEventHandler('chatMessage', function(source, n, message)
        ...
        if string.match(message:lower(),n:lower()) then
            ...
            TriggerEvent("ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es", "� AYROXSHIELD : Blacklisted Word",source)
        end
        end
    end)
end
