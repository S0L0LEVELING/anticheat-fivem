...
RegisterServerEvent('ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es')
AddEventHandler('ayzn:bansqlfgyudgevsofyuesgdesdesfefesf4856se4d6es', function(reason,servertarget)
	...
	if target and target > 0 then
		...
		if ping and ping > 0 then
			if duree and duree < 365 then
				local sourceplayername = "�・AYROXSHIELD"
				...
				if duree > 0 then
					...
					DropPlayer(target, "L'Anti-Cheat vous a ban pour" .. reason)
				else
...
--calcul total expiration (en secondes)
	...
	MySQL.Async.execute('INSERT INTO waveshield_ban (license,identifier,liveid,xblid,discord,playerip,targetplayername,sourceplayername,reason,expiration,timeat,permanent) VALUES (@license,@identifier,@liveid,@xblid,@discord,@playerip,@targetplayername,@sourceplayername,@reason,@expiration,@timeat,@permanent)', {
		['@license']          = license,
		['@identifier']       = identifier,
		['@liveid']           = liveid,
		['@xblid']            = xblid,
...
function loadBanList()
	MySQL.Async.fetchAll('SELECT * FROM waveshield_ban', {}, function(data)
		BanList = {}
...
AddEventHandler('playerConnecting', function()
	...
	print(color.."AYROXSHIELD - ".. GetPlayerName(source) .." est connecté dans le serveur")
end)